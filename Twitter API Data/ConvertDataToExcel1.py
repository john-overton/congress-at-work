...
def round_up_to_1min(dt):
    return dt - timedelta(minutes=dt.minute % -1,
                          seconds=dt.second,
                          microseconds=dt.microsecond)

# Apply the rounding function to create a new column for grouping
df['group_start'] = df['start'].apply(round_up_to_1min)
...
with pd.ExcelWriter('tweet_counts_1min_grouped.xlsx') as writer:
...
print("Excel file 'tweet_counts_1min_grouped.xlsx' has been created successfully.")