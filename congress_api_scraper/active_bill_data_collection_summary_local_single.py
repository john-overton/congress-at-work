...
log_file = os.path.join(log_dir, 'active_bill_data_collection_summary_local_single.log')
...
def construct_prompt(congress, bill_type, bill_number, bill_title, previous_context, bill_text, next_context, bill_actions, text_part, total_parts):
    ...
    prompt = f"""<|Assistant|>
You are a legislative analyst focused on providing precise, factual summaries of specific portions of bills. Your task is to analyze and summarize the provided bill text section, considering its position within the context provided if applicable. Write a 4-5 paragraph summary that captures the essential facts from the specific text section provided.
